...
export const polkadotService = {
  ...
  connect: async (source: string): Promise<WalletData> => {
    ...
    const extension: InjectedExtension = await injectedWeb3[source].enable('Pluto Protocol');
    ...
    // Note: In a real app, you would query chain balance here. We simulate balances for the UI.
    return {
  ...
  signMessage: async (address: string, source: string, message: string): Promise<string> => {
    ...
    const extension = await injectedWeb3[source].enable('Pluto Protocol');
